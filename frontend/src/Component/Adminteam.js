import React from "react";
import Navbar from "./Navbar";
function Adminteam() {
    return (
        <div>
            <Navbar/>
        <div class="py-20 bg-gray-50 bg-#ebd5f9">
            <div class="container mx-auto px-6 md:px-12 xl:px-32">
                <div class="mb-16 text-center">
                    <h2 class="mb-4 text-center text-2xl text-gray-900 font-bold md:text-4xl">Tailus blocks leadership</h2>
                    <p class="text-gray-600 lg:w-8/12 lg:mx-auto">Tailus prides itself not only on award-winning technology, but also on the talent of its people of some of the brightest minds and most experienced executives in business.</p>
                </div>
                <div class="grid gap-12 items-center md:grid-cols-3">
                    <div class="space-y-4 text-center">
                        <img class="w-64 h-64 mx-auto object-cover rounded-xl md:w-40 md:h-40 lg:w-64 lg:h-64"
                            />
                        <div>
                            <h4 class="text-2xl">Pradeep</h4>
                            <span class="block text-sm text-gray-500">Founder</span>
                        </div>
                    </div>
                    <div class="space-y-4 text-center">
                        <img class="w-64 h-64 mx-auto object-cover rounded-xl md:w-40 md:h-40 lg:w-64 lg:h-64"
                            />
                        <div>
                            <h4 class="text-2xl">Sai Kumar Puramvar</h4>
                            <span class="block text-sm text-gray-500">Founder</span>
                        </div>
                    </div>
                    <div class="space-y-4 text-center">
                        <img class="w-64 h-64 mx-auto object-cover rounded-xl md:w-40 md:h-40 lg:w-64 lg:h-64"
                            />
                        <div>
                            <h4 class="text-2xl">Mythrika Nari</h4>
                            <span class="block text-sm text-gray-500">Founder</span>
                        </div>
                    </div>
                    <div class="space-y-4 text-center">
                        <img class="w-64 h-64 mx-auto object-cover rounded-xl md:w-48 md:h-64 lg:w-64 lg:h-80"
                            src="" />
                        <div>
                            <h4 class="text-2xl">Neha Megaraju</h4>
                            <span class="block text-sm text-gray-500">Founder</span>
                        </div>
                    </div>
                    <div class="space-y-4 text-center">
                        <img class="w-64 h-64 mx-auto object-cover rounded-xl md:w-40 md:h-40 lg:w-64 lg:h-64"
                            src="https://media.licdn.com/dms/image/D5603AQEbVKGCe7otZQ/profile-displayphoto-shrink_100_100/0/1676270372253?e=1706140800&v=beta&t=qkQozMUWMrbnYqncxOHOhxVGIxHqNNupvnNnbVB4CRo" /><div>
                            <h4 class="text-2xl">Bhanu Prakash Seetha</h4>
                            <span class="block text-sm text-gray-500">Founder</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        </div>
    );
}
export default Adminteam